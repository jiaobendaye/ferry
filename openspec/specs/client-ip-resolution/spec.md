...
Resolve the real client IP used for ACL enforcement and bandwidth limiting when the server runs behind a proxy: select the trusted-hop entry of `X-Forwarded-For` (counted from the right, spoof-resistant), fall back to the socket peer address, and normalize IPv4/IPv6 forms to a single identity.
